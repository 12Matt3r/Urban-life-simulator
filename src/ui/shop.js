...
/**
 * Shop
 * A simple UI module for an in-game shop.
 */
export const Shop = {
  gameManager: null,
  gameAudio: null,
  ...
  show: function() {
    ...
    document.getElementById('buy-water-btn').onclick = () => this.buyWater();
    document.getElementById('close-shop-btn').onclick = () => this.hide();
  },
  ...
  buyWater: function() {
    ...
    if (playerMoney >= itemCost) {
      ...
      // The original sfx ids were 'ui_confirm' and 'ui_error' which don't exist in assets.json.
      // Using 'success' for now.
      this.gameAudio.playSfx('success');
      alert('You bought a bottle of water.');
    } else {
      // Using a generic click as an error sound.
      this.gameAudio.playSfx('ui_click');
