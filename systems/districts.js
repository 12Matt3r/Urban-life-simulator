 ...
 */
export function suggestNextDistrict(current, roleTags, seed) {
  var rng = createRNG(seed || Date.now());
  var tags = (roleTags || []).map(function(s) { return String(s).toLowerCase(); });
  var pool = DEFAULTS.filter(function(d) { return d !== current; });
  ...
  if (boost.length) {
    var combinedPool = pool.concat(boost).concat(boost); // duplicate boosts for weight
    pool = uniq(combinedPool);
  }
  return rng.pick(pool.length ? pool : DEFAULTS);
}

function hasAny(tags, list) {
  return list.some(function(x) { return tags.indexOf(x) !== -1; });
}
function uniq(arr) {
  var seen = {};
